...
Record는 원본 기록을 보존하고, 주간 단위로 반복 조회되는 값은 `weekly_user_stats` read model로 누적한다. 저장과 집계 반영은 outbox를 통해 분리되어 있으며, 저장 성공과 집계 반영은 같은 순간을 보장하지 않는다.
...
- `weekly_user_stats`에 주간 기록 수, 거리, 시간, 평균 페이스, 티어 점수 집계를 누적한다.
...
`WeeklyUserStats`는 사용자와 주차 기준으로 누적되는 주간 집계 row다.
...
- 원본 기록을 삭제하거나 수정하는 기능을 추가할 때는 `weekly_user_stats`를 차감하거나 재계산하는 정책이 함께 필요하다.
