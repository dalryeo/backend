...
public class RunningRecordValidator {
    ...
    public void validate(RunningRecordRequest request, LocalDateTime currentTime) {
    ...
    private void validateFutureTime(RunningRecordRequest request, LocalDateTime currentTime) {
        LocalDateTime maxAllowedTime = currentTime.plusSeconds(FUTURE_TIME_ALLOWANCE_SECONDS);
        if (request.getStartAt().isAfter(maxAllowedTime) || request.getEndAt().isAfter(maxAllowedTime)) {
