...
class GlobalExceptionHandlerTest {
    ...
    private static final ErrorCode SERVER_ERROR_CODE = new TestErrorCode(
    ...
    void handleBusinessException_usesExceptionMessageWhenProvided() {
        ResponseEntity<CommonResponse<Map<String, Object>>> response =
                handler.handleBusinessException(
                        new AuthException(AuthErrorCode.REFRESH_TOKEN_INVALID, CUSTOM_AUTH_MESSAGE),
                        request("/auth/token/refresh")
                );
    ...
    void handleBusinessException_returnsConfiguredStatusCodeAndErrorCodeForRecordValidationException() {
        ResponseEntity<CommonResponse<Map<String, Object>>> response =
                handler.handleBusinessException(
                        new RecordValidationException(RecordValidationErrorCode.INVALID_DURATION),
                        request("/records")
                );
    ...
    @Test
    void handleBusinessException_logsOperationalErrorContext(CapturedOutput output) {
        handler.handleBusinessException(
                new AuthException(AuthErrorCode.REFRESH_TOKEN_INVALID),
                request("/auth/token/refresh")
        );
        ...
        assertThat(output.getOut())
                .contains("api.error code=AC-006")
                .contains("status=401")
                .contains("path=/auth/token/refresh")
                .contains("exception=AuthException");
    }

    @Test
    void handleBusinessException_logsStackTraceForServerError(CapturedOutput output) {
        handler.handleBusinessException(
                new TestBusinessException(SERVER_ERROR_CODE),
                request("/records")
        );

        assertThat(output.getOut())
                .contains("api.error code=TEST-500")
                .contains("status=500")
                .contains("path=/records")
                .contains("exception=TestBusinessException")
                .contains("at com.ohgiraffers.dalryeo.common.exception.GlobalExceptionHandlerTest");
    }
    ...
    void handleHttpMessageNotReadableException_returnsFieldErrorWhenOffsetDateTimeFormatIsInvalid() {
        ...
        ResponseEntity<CommonResponse<Map<String, Object>>> response =
                handler.handleHttpMessageNotReadableException(messageNotReadableException(cause), request("/records"));
    ...
    void handleHttpMessageNotReadableException_returnsFieldErrorWhenLocalDateFormatIsInvalid() {
        ...
        ResponseEntity<CommonResponse<Map<String, Object>>> response =
                handler.handleHttpMessageNotReadableException(messageNotReadableException(cause), request("/onboarding"));
    ...
    private record TestErrorCode(HttpStatus status, String code, String message) implements ErrorCode {
        @Override
    ...
    private static class TestBusinessException extends BusinessException {
        private TestBusinessException(ErrorCode errorCode) {
            super(errorCode);
        }
    }
}
